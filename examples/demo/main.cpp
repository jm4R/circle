...

{
    ...
    // rectangle
    auto r1 = window.content_item.add<rectangle>();
    ...
    auto img = window.content_item.add<image>();
    img->source = "test.png";

    auto txt = window.content_item.add<text>();
    txt->font.point_size = 24;
    txt->color = BIND((txt->x, x), color(80, x / 2, 80, 255 - x / 2));
    txt->value = "Circle it is!";
